...
#  </copyright>
#---------------------------------------------------------------------------
# @(#) MQMBID sn=p920-L200709 su=_he1cUsHdEeqhru3bNhtEjw pn=install/unix/genmqpkg.sh
#---------------------------------------------------------------------------
# File Description :
...
askquestion incmqft "Does the runtime require Managed File Transfer [Y/N]? " "$mqdir/mqft" "FTAgent FTBase FTLogger FTService FTTools"
...
if [ $incamqp -eq 0 ] && [ $incmqbc -eq 0 ] && [ $incmqft -eq 0 ] && [ $incmqsf -eq 0 ] && [ $incmqxr -eq 0 ] && [ $incweb -eq 0 ]
then
